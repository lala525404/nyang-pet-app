...
const DEFAULT_SETTINGS: Settings = {
  ...
  distractInterval: 30, imageRotateInterval: 60, useCustomMode: false, moveSpeed: 5,
};
...
function useScreenMovement(speedRef: React.MutableRefObject<number>) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (s.wx < 0)        { s.wx = 0;        s.vx =  Math.abs(s.vx) * br(0.6,1.0); }
      if (s.wx > sw - 190) { s.wx = sw - 190; s.vx = -Math.abs(s.vx) * br(0.6,1.0); }
      if (s.wy < 0)        { s.wy = 0;        s.vy =  Math.abs(s.vy) * br(0.6,1.0); }
      if (s.wy > sh - 190) { s.wy = sh - 190; s.vy = -Math.abs(s.vy) * br(0.6,1.0); }
...
export default function PetPage() {
  ...
  // speedRef를 항상 최신 settings.moveSpeed와 동기화
  useEffect(() => { speedRef.current = settings.moveSpeed || 5; }, [settings.moveSpeed]);

  const { displayTransform, reset, triggerEscape, stateRef, draggingRef, setOnWallHit } = useScreenMovement(speedRef);
  ...
  }, [triggerEscape, showBubble, animal, isCustomMode]);
    ...
    <div className="pet-root">
      <div className="pet-container">
        ...
        <div className="cat-wrapper">
          ...
          >
            {ambushEmoji ? (
              <CatEmoji emoji={ambushEmoji} size={96} className="cat-emoji cat-emoji--ambush" />
            ) : (
              <CatEmoji
                emoji={currentEmoji}
                size={64}
